...
private:
    ...
    unique_ptr<NeuralNetPlayer> learner_for(BgNet& nn, const TrainingOptions& opts)
    {
        ...
        if (auto p = dynamic_cast<Fc_Sig_Bias_H30_I3*>(&nn))
            return make_unique<Learner<Fc_Sig_Bias_H30_I3> > (*p, opts.alpha, opts.lambda, opts.wdual, opts.decay, opts.batchsize, opts.momentum);
